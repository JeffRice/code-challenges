...
while(numberOfStacks){
    ...
    let dynamicArray = lines.map(x => x.charAt(1 + (4 * numberOfStacks))).filter(x => x !== ' ').reverse()
...
for (var i = 0; i < numberOfInstructions; i++){
    ...
    let moveTarget = Number(splitOnSpace[5]) - 1
        ...
        moveCrate(moveSource, moveTarget, moveQuantity)
...
function moveCrate(moveSource, moveTarget, moveQuantity) {
    let sourceLength = stackValues[moveSource].length
    let movedCrates = stackValues[moveSource].slice(sourceLength - moveQuantity, sourceLength)
    stackValues[moveSource] = stackValues[moveSource].slice(0, sourceLength - moveQuantity)
 //   stackValues[moveTarget].push(movedCrates)

           for(var i = 0; i < moveQuantity; i++){
            stackValues[moveTarget].push(movedCrates[i])
        }
